...
export async function analyzeImages(ai: GoogleGenAI, images: UploadedImage[], isProMode: boolean): Promise<AnalysisReport> {
    console.groupCollapsed(`[API] Calling Gemini for multi-image analysis`);
...
- [Liste hier Beobachtungen zu Nicht-Wildtieren mit Bildreferenzen (Bild C) auf.]`;
    ...
    console.log(`Sending ${images.length} images for analysis using ${modelName} with thinkingBudget: ${thinkingBudget}.`);
    ...
    try {
        ...
        // Clean up potential preamble before the main report title
        ...
        console.log(`[API] Analysis received. Prompt tokens: ${promptTokenCount}, Candidates tokens: ${candidatesTokenCount}`);
        return { 
            markdownContent, 
            promptTokenCount,
        ...
        console.error("[API] Error during image analysis:", error);
        throw error;
    } finally {
        console.groupEnd();
    }
